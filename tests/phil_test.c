...
/**
 * This file uses the CUnit framework to unit test the various functions of
 * Philphix. We recommend you make use of these tests to assist with writing and
 * debugging your functions. You are also encouraged to add your own tests (see
 * main()).
 *
 * Note that passing all the unit tests for a function does NOT guarantee that
 * that function is fully correct. These tests are largely sanity tests and
 * basic cases, but may not cover all edge cases or possible scenarios. Again,
 * you are encouraged to write your own tests for these cases.
 */
 ...
 * This is NOT the proper way to free a HashTable, but for small scale unit
 * tests it doesn't particuarly matter. */
int clean_hashtb() {
    free(dictionary);
    dictionary = NULL;
    return 0;
...
int main() {
    ...
    // Task 3
    CU_pSuite task3Suite =
        CU_add_suite("Task 3", task3_setup_teardown, task3_setup_teardown);
    if (pSuite == NULL) {
        CU_cleanup_registry();
        return CU_get_error();
    }
    ...
    if ((CU_add_test(task3Suite, "Empty dictionary", task3_test_empty) == NULL)
        ...
        || (CU_add_test(task3Suite, "Simple without newline",
                        task3_test_simple_no_newline)
            == NULL)
        || (CU_add_test(task3Suite, "Alphanumeric key", task3_test_alphanum)
            == NULL)
        || (CU_add_test(task3Suite, "Multiple entries", task3_test_multiline)
            == NULL)
        || (CU_add_test(task3Suite, "Non-alphanumeric value",
                        task3_test_punctuation)
            == NULL)
        || (CU_add_test(task3Suite, "Tabs and spaces", task3_test_tabs)
            == NULL)) {
        CU_cleanup_registry();
        return CU_get_error();
    }
